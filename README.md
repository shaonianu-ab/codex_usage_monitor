...
- 后台刷新时，若可用 reset credit 数量未减少且本周剩余额度至少增加 2 个百分点，会发送“检测到额度重置！”的系统通知。
...
应用启动时会请求系统通知权限。只有本次与上次刷新都成功获得周额度和 reset credit 数据、可用 reset credit 数量未减少，并且本周剩余百分比至少增加 2 个百分点时，才会发送额度重置通知；这可避免网络、单个接口失败或微小数据波动导致误报。
